...
_FIXTURES = sorted((Path(__file__).parent / "fixtures").glob("*.md"))


def _fmt(text: str) -> str:
    return mdformat.text(text, options={"wrap": 120}, extensions=_EXTENSIONS)


@pytest.mark.parametrize("fixture", _FIXTURES, ids=lambda p: p.stem)
def test_fixture_is_stable(fixture: Path) -> None:
    formatted = _fmt(fixture.read_text(encoding="utf-8"))
    assert _fmt(formatted) == formatted


@pytest.mark.parametrize("fixture", _FIXTURES, ids=lambda p: p.stem)
def test_fixture_preserves_shortcodes_and_bodies(fixture: Path) -> None:
    out = _fmt(fixture.read_text(encoding="utf-8"))
