...
def _block_rule(state: StateBlock, start_line: int, end_line: int, silent: bool) -> bool:  # noqa: FBT001 - markdown-it-py rule signature
    opening = _open_delimiter(_line(state, start_line))
    if opening is None:
        return False
    ...
    if silent:
        return True
    body = state.getLines(tag_end + 1, line, state.blkIndent, keepLastLF=False)
    token = state.push("zola_shortcode_block", "", 0)
    token.content = _wrap_block(start.inner, body, escaped=escaped)
...
def _wrap_block(start_inner: str, body: str, *, escaped: bool) -> str:
    open_delim, close_delim, end_tag = ("{%/*", "*/%}", "{%/* end */%}") if escaped else ("{%", "%}", "{% end %}")
    start_tag = f"{open_delim} {start_inner} {close_delim}"
    if body:
        return f"{start_tag}\n{body}\n{end_tag}"
    return f"{start_tag}\n{end_tag}"
